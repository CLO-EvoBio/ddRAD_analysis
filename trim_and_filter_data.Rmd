---
title: "Trimming and Filtering ddRAD data"
output: github_document
---

**Reserve a general machine at BioHPC and connect to this machine** 

In order to compute we will need to reserve a machine at the BioHPC for our computing. For ddRAD analysis we can use the general machines (cheapest, but there are only 2 available and so they are often hard to get) or the medium gen1 machines. Connect to this machine:

```{r, eval = FALSE}
ssh USER_NAME@MACHINE_NAME.biohpc.cornell.edu
```

**navigate to the working directory and make directory for the analysis** 
```{r, eval = FALSE}
cd /workdir
...
To do this we can either run fastQC on each file or we can write a script to run fastQC on all of our data.
...
```
You will need to transfer this script file to /workdir on the cluster using Filezilla. Make sure that the script is in the same directory as the data. Run the script.

```{r, eval=FALSE}
bash fastqc.sh &
```

This should output an html file with results that can be transfered back to your computer and viewed in a browser.

# Trim and Filter the data

We will use the `fastx_trimmer` and `fastq_quality_filter` functions from the `FASTX-Toolkit` to make sure that we are using good quality data for the STACKS analysis.
...
Create a script (trimfilter.sh, see example below) so that all steps can be completed without much user input.
...
-   The hangup (HUP) signal is normally sent to a process to inform it that the user has logged off (or "hung up") 
-   The HUP signal is intercepted by nohup, allowing the process to continue running if you logout
-   All output, including any error messages, will be written to the file nohup.out in the directory where you run this command. #each time you run a command using nohup, if there is already a nohuop.out file, it will add the output to the end of that file. 
-   I usually rename or delete the existing nohup before running the next command making it easier to check the files later.
